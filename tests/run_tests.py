...
def run_tests(test_type="all", verbose=False):
    ...
    if test_type == "all" or test_type == "legacy":
        test_files.extend(["tests/test_config.py", "tests/test_extractors.py", "tests/test_mcp_server.py"])

    # Run pytest
    pytest_args = ["python", "-m", "pytest", "-v" if verbose else "", "--tb=short", "--strict-markers", "--disable-warnings"]
...
def run_legacy_tests():
    ...
    legacy_tests = ["tests/test_config.py", "tests/test_extractors.py", "tests/test_mcp_server.py"]
    ...
    for test_file in legacy_tests:
        ...
        if test_path.exists():
            print(f"\nRunning {test_file}...")
            try:
                result = subprocess.run([sys.executable, str(test_path)], cwd=project_root, capture_output=True, text=True)
...
def main():
    ...
    parser.add_argument("--type", choices=["all", "unit", "integration", "legacy"], default="all", help="Type of tests to run")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--legacy-only", action="store_true", help="Run only legacy test scripts")
