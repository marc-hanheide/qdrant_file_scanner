...
class ReRanker:
    ...
    def rerank_results(self, query: str, results: List[Dict], target_limit: int) -> List[Dict]:
        ...
        try:
            ...
            self.logger.info(
                f"Re-ranked {len(results)} -> {len(final_results)} results "
    ...
    def check_and_unload_idle_model(self) -> bool:
        ...
        if (self.cross_encoder is not None and 
            self.model_last_used is not None and 
            (datetime.now() - self.model_last_used).total_seconds() > self.unload_after_minutes * 60):
