...
class RAGSearchCLI:
    ...
    def search_by_query(self, query: str, limit: int = 10, min_score: float = 0.0, 
                       include_deleted: bool = False, glob_pattern: str = None,
    ...
    def search_by_example(self, example_file: str, limit: int = 10, min_score: float = 0.0,
                         include_deleted: bool = False, glob_pattern: str = None,
                         start_date: str = None, end_date: str = None) -> List[Dict]:
    ...
    def search_by_glob(self, glob_pattern: str, start_date: str = None, 
...
Examples:
  ...
  rag-search "temp file" --delete --dry-run
    ...
    parser.add_argument('query', nargs='?', 
                       help='Search query for semantic search (default mode)')
    
    # Search type options
    search_group = parser.add_mutually_exclusive_group()
    search_group.add_argument('--example', '-e', type=str,
                             help='Path to example file for similarity search')
    search_group.add_argument('--glob-only', type=str,
                             help='Search only by glob pattern (no semantic search)')
    ...
    parser.add_argument('--start-date', type=str,
                       help='Start date filter (YYYY-MM-DD, "today", "1 week ago", etc.)')
    parser.add_argument('--end-date', type=str,
                       help='End date filter (YYYY-MM-DD, "today", "1 week ago", etc.)')
    ...
    parser.add_argument('--delete', '-d', action='store_true',
                       help='Delete found documents from the database')
...
def main():
    ...
    try:
        ...
        if query_text:
            ...
            # Pure glob search
            results = cli.search_by_glob(
                glob_pattern=args.glob_only,
                start_date=args.start_date,
                end_date=args.end_date
            )
